...
def build_split(
    ...
    text_mode: str = "none",
    include_part_seg_file: bool = False,
) -> Dict:
    """Build a lightweight Talk2DINO-style meta file.

    text_mode:
        none     - store only ids/names required by DINO crop extraction and LLaMA3 injection.
        imagenet - also store old CLIP/ImageNet-template caption and part_caption fields.
    """
    text_mode = str(text_mode).lower()
    if text_mode not in {"none", "imagenet"}:
        raise ValueError(f"Unsupported text_mode={text_mode}. Use none/imagenet.")
    ...
    for img_id, mask_path in enumerate(tqdm(mask_files, desc=f"Building {split}")):
        ...
        if include_part_seg_file:
            # Convenience only: downstream code can still derive this from seg_file_name when paths follow
            # annotations_detectron2_obj -> annotations_detectron2_part.
            part_mask_path = Path(str(mask_path).replace("annotations_detectron2_obj", "annotations_detectron2_part"))
            image_record["part_seg_file_name"] = str(part_mask_path)
        ...
        for class_idx in present_ids:
            ...
            if text_mode == "imagenet":
                ann_record["caption"] = build_prompts(class_name)
                ann_record["part_caption"] = [build_prompts(part_name) for part_name in part_class_name]
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="none",
        help="none: lightweight meta for DINO + LLaMA3; imagenet: also write old caption/part_caption prompt fields.",
    )
    parser.add_argument(
        "--include_part_seg_file",
        action="store_true",
        help="Also store part_seg_file_name in image records if paths follow annotations_detectron2_obj -> annotations_detectron2_part.",
    )
    parser.add_argument(
        "--output_name",
        type=str,
    ...
    for split in args.splits:
        result = build_split(
            ...
            include_part_seg_file=args.include_part_seg_file,
        )
