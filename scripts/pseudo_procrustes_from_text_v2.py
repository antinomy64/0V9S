...
def evaluate_with_transform(
    *,
    ...
    # Raw text may be 512D while pseudo/GT are 768D.
    # Direct cross-cosine retrieval BEFORE mapping is only meaningful when dimensions match.
    if text.shape[1] == pseudo.shape[1]:
        results["text_vs_pseudo_before"] = retrieval_metrics(
        ...
        print("=" * 100)
        print(
            f"[skip] {split_name}: {text_source} text vs pseudo BEFORE pseudo-Procrustes "
            f"because dims differ: text={text.shape[1]}, pseudo={pseudo.shape[1]}"
        )
        results["text_vs_pseudo_before"] = {
            "skipped": True,
            "reason": "dimension_mismatch",
            "text_dim": int(text.shape[1]),
            "target_dim": int(pseudo.shape[1]),
        }

    if text.shape[1] == gt.shape[1]:
        results["text_vs_gt_before"] = retrieval_metrics(
        ...
        print("=" * 100)
        print(
            f"[skip] {split_name}: {text_source} text vs GT BEFORE pseudo-Procrustes "
            f"because dims differ: text={text.shape[1]}, GT={gt.shape[1]}"
        )
        results["text_vs_gt_before"] = {
            "skipped": True,
            "reason": "dimension_mismatch",
            "text_dim": int(text.shape[1]),
            "target_dim": int(gt.shape[1]),
        }

    # AFTER mapping, text_mapped should live in visual/DINO dimension.
    results["mapped_text_vs_pseudo_after"] = retrieval_metrics(
        text_mapped, pseudo, part_ids, part_names,
        label=f"{split_name}: mapped {text_source} text vs pseudo AFTER pseudo-Procrustes",
        print_per_part=print_per_part,
    )
    results["mapped_text_vs_gt_after"] = retrieval_metrics(
