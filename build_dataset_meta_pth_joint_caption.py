...
def build_joint_caption(class_names: List[str]) -> str:
    """Build one caption per image from all present class names."""
    if not class_names:
        return "a photo."
    if len(class_names) == 1:
        return f"a photo of {class_names[0]}."
    if len(class_names) == 2:
        return f"a photo of {class_names[0]} and {class_names[1]}."
    return f"a photo of {', '.join(class_names[:-1])}, and {class_names[-1]}."
...
def build_split(
    ...
    for img_id, mask_path in enumerate(tqdm(mask_files, desc=f"Building {split}")):
        ...
        class_names = [CLASSES[class_idx] for class_idx in present_ids]

        annotations.append(
            {
                "id": len(annotations),
                "image_id": current_image_id,
                "category_id": present_ids,
                "class_name": class_names,
                "caption": build_joint_caption(class_names),
            }
        )
