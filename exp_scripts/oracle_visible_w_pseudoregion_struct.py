...
# -*- coding: utf-8 -*-
"""
Clean test15 oracle-visible orthogonal W training, pseudo-region supervision + structure constraint version.
...
-------------------
  ...
  local_loss = mean(1 - cos(z[k], proto_k))
  structure_loss = pairwise cosine-structure loss between text points z and pseudo-region prototypes
  loss = local_loss + lambda_structure * structure_loss
...
This script intentionally removes the redundant modes from earlier scripts:
  ...
  - optional structure loss uses pseudo-region prototypes, not GT patch masks
...
def offdiag_mask(n: int, device: torch.device) -> torch.Tensor:
    return ~torch.eye(n, dtype=torch.bool, device=device)


def structure_loss_pairwise_cos(
    z_text: torch.Tensor,
    z_proto: torch.Tensor,
    mode: str = "cos_mse",
) -> torch.Tensor:
    """Differentiable near-isometry loss over visible parts.

    z_text: [K, D], text-side points after W.
    z_proto: [K, D], pseudo-region visual prototypes.

    This is a differentiable surrogate for Spearman/structure matching:
      S_T[i,j] = cos(z_text_i, z_text_j)
      S_V[i,j] = cos(z_proto_i, z_proto_j)
      loss = MSE(offdiag(S_T), offdiag(S_V))

    It does not use GT masks. The visual structure comes from pseudo-region prototypes.
    """
    K = z_text.shape[0]
    if K <= 1:
        return z_text.new_tensor(0.0)
    st = pairwise_cosine_matrix(z_text)
    sv = pairwise_cosine_matrix(z_proto)
    mask = offdiag_mask(K, z_text.device)
    vt = st[mask]
    vv = sv[mask]
    if mode == "centered_cos_mse":
        vt = vt - vt.mean()
        vv = vv - vv.mean()
    elif mode != "cos_mse":
        raise ValueError(f"Unknown structure_mode={mode}")
    return torch.mean((vt - vv) ** 2)
...
def sample_region_loss(
    ...
    region_protos: List[torch.Tensor] = []
    ...
    proto_stack = torch.stack(region_protos, dim=0)
    struct_loss = structure_loss_pairwise_cos(z_w, proto_stack, mode=args.structure_mode)
    loss = (
        region_loss
        + float(args.anchor_loss_weight) * anchor_loss
        + float(args.structure_loss_weight) * struct_loss
    )

    return loss, {
        "region_loss": float(region_loss.detach().cpu().item()),
        "anchor_loss": float(anchor_loss.detach().cpu().item()),
        "structure_loss": float(struct_loss.detach().cpu().item()),
...
def batch_loss(model, batch: Dict, W: torch.Tensor, args) -> Tuple[torch.Tensor, Dict[str, float]]:
    ...
    for b in range(part_z0.shape[0]):
        ...
        region_loss_sum += stat_b.get("region_loss", 0.0)
        anchor_loss_sum += stat_b.get("anchor_loss", 0.0)
        structure_loss_sum += stat_b.get("structure_loss", 0.0)
        loss_stat_count += 1.0
    ...
    return torch.stack(losses).mean(), {
        ...
        "region_loss": float(region_loss_sum) / max(float(loss_stat_count), 1.0),
        "anchor_loss": float(anchor_loss_sum) / max(float(loss_stat_count), 1.0),
        "structure_loss": float(structure_loss_sum) / max(float(loss_stat_count), 1.0),
    }
...
def train_W(model, train_loader, device: torch.device, dim: int, args) -> Tuple[torch.Tensor, List[Dict]]:
    ...
    for epoch in range(args.epochs):
        ...
        for step, batch in enumerate(pbar):
            ...
            total_region_loss += info.get("region_loss", 0.0)
            total_anchor_loss += info.get("anchor_loss", 0.0)
            total_structure_loss += info.get("structure_loss", 0.0)
            steps += 1
            pbar.set_postfix(
                ...
                str=f"{total_structure_loss / max(steps, 1):.3f}",
            )
        ...
        record = {
            ...
            "region_loss": total_region_loss / max(steps, 1),
            "anchor_loss": total_anchor_loss / max(steps, 1),
            "structure_loss": total_structure_loss / max(steps, 1),
            "steps": steps,
        }
...
def parse_args():
    ...
    parser.add_argument("--structure_loss_weight", type=float, default=0.0, help="weight for pairwise structure/isometry loss between text points and pseudo-region prototypes")
    parser.add_argument("--structure_mode", type=str, default="cos_mse", choices=["cos_mse", "centered_cos_mse"], help="structure loss variant")
...
def main() -> None:
    ...
    print(f"[setup] structure: weight={args.structure_loss_weight}, mode={args.structure_mode}")
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump({
            ...
            "notes": {
                ...
                "objective": "relative greedy seed + high-confidence pseudo-region prototype cost + optional pairwise structure/isometry loss",
            },
