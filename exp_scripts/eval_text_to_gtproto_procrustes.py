...
4) Evaluate on validation object crops with oracle-visible candidates:
     For each val crop, candidates are the GT-visible part ids in that crop.
     Classify object-mask patches by nearest prototype among those candidates.
     Report object-crop PATCH-LEVEL mIoU. This is not the official full-resolution
     segmentation evaluator, but is directly comparable to eval_gtproto_patch_miou.py.

Important supervision note
--------------------------
This is an upper-bound diagnostic. Train GT masks are used to build the visual
prototype target bank. Val GT masks are used only to choose oracle-visible candidate
parts and to compute evaluation metrics.
"""
...
def eval_prototype_bank(val_loader, proto_bank: Dict[int, torch.Tensor], device: torch.device, args, desc: str):
    ...
    for step, batch in enumerate(pbar):
        ...
        for b in range(batch["patch_tokens"].shape[0]):
            ...
            if sample is None:
                skipped_samples += 1
                continue
            p_obj, gt_vis_obj, part_ids, _, _ = sample
    ...
    out.update({
        ...
        "missing_part_instances_without_proto": missing_part_instances,
    })
    return out
...
def main() -> None:
    ...
    result = {
        ...
        "notes": {
            ...
            "train_visual_gtproto": "train GT masks build global visual prototype bank; val GT masks used only for candidate filtering/eval",
    ...
    print("Key numbers [patch-level oracle-visible]:")
