    ...
    for i in tqdm(range(n_batch)):
        ...
        if model_type == 'clip':
            if use_caption_ensemble:
                for j in range(start, end):
                    caption = data['annotations'][j]['caption']
                    if isinstance(caption, (list, tuple)):
                        data['annotations'][j][field_name] = encode_caption_ensemble(model, list(caption), device)
                    else:
                        inputs = clip.tokenize([caption], truncate=True).to(device)
                        with torch.no_grad():
                            outputs = model.encode_text(inputs)
                        data['annotations'][j][field_name] = outputs[0].to('cpu')
            else:
