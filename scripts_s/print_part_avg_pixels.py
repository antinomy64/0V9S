...
def main():
    parser = argparse.ArgumentParser(
        description="Print average pixel count per object crop for each part."
    )
    ...
    print("| part_id | part_name | avg_pixel_count_per_obj_crop |")
    print("|---:|---|---:|")
    for r in rows:
        print(
    ...
    if args.out_csv is not None:
        ...
        with open(out_path, "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(
                f,
                fieldnames=["part_id", "part_name", "avg_pixel_count_per_obj_crop"],
            )
            writer.writeheader()
